...
import {
  ...
  CheckCircle2, AlertTriangle, Clock, BookOpen, User, FileText, HelpCircle
} from 'lucide-react';
...
export const ReportView = () => {
  ...
  const reportDoc = response?.data?.data || response?.data;
  const analysis = reportDoc?.analysis || {};
  const metadata = reportDoc?.metadata || {};
  ...
  const argumentStructure = analysis.argumentStructure || {};
  const claimSummaryStats = analysis.claimSummaryStats || {};
  ...
  const summaryText = articleContext.oneSentenceSummary || summary;
  ...
  if (isError || !reportDoc) {
    return (
      <div className="flex h-[80vh] items-center justify-center flex-col gap-4">
        <div className="text-red-500 font-semibold text-xl">Failed to load report</div>
        <p className="text-prism-text-secondary">{error?.message || "Report could not be found."}</p>
        <Link to="/"><Button variant="outline">Return to Workspace</Button></Link>
      </div>
    );
  }
  ...
  return (
    <div className="relative max-w-6xl mx-auto pb-20 pt-6 space-y-8">
      ...
      >
        <Link to="/" className="flex items-center text-prism-text-secondary hover:text-white transition-colors group">
          ...
          <span className="tracking-wide uppercase text-sm font-medium">Workspace</span>
        </Link>
        <div className="flex items-center gap-3">
          ...
          <Button variant="ghost" size="sm" onClick={handleShare} className="gap-2 hover:bg-white/10 text-xs">
            {copied ? <Check size={16} className="text-emerald-400" /> : <Share size={16} />}
            <span>{copied ? "Link Copied!" : "Share"}</span>
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} className="gap-2 border-white/20 hover:bg-white/5 text-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
          className="lg:col-span-4 glass-panel rounded-3xl p-5 flex flex-col items-center justify-center relative overflow-hidden border-t border-white/10 text-center"
        >
          ...
          <Badge
            ...
            className="text-xs px-4 py-1 mt-4 mb-2 tracking-widest uppercase"
          >
            {verdict}
          </Badge>
          <span className="text-[11px] font-mono text-white/40">
            System Confidence: {briefing.confidence || 85}%
          </span>
        </motion.div>
        ...
        <motion.div
          ...
          className="lg:col-span-4 glass-panel rounded-3xl p-5 flex flex-col justify-between border-t border-white/10 space-y-4"
        >
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white/[0.03] border border-white/8 p-3 rounded-xl">
                <span className="text-[10px] font-mono uppercase text-white/40 block mb-1 flex items-center gap-1">
                  <FileText size={10} /> Type
                </span>
                <span className="text-xs font-semibold text-white truncate block">{detectedIntent}</span>
              </div>
              <div className="bg-white/[0.03] border border-white/8 p-3 rounded-xl">
                <span className="text-[10px] font-mono uppercase text-white/40 block mb-1 flex items-center gap-1">
                  <BookOpen size={10} /> Topic
                </span>
                <span className="text-xs font-semibold text-white truncate block">{primaryTopic}</span>
              </div>
              <div className="bg-white/[0.03] border border-white/8 p-3 rounded-xl">
                <span className="text-[10px] font-mono uppercase text-white/40 block mb-1 flex items-center gap-1">
              ...
              <div className="bg-white/[0.03] border border-white/8 p-3 rounded-xl">
                ...
                <span className="text-xs font-semibold text-white block">{readingTime} min read</span>
              </div>
            </div>
          </div>
        ...
        <motion.div
          ...
          className="lg:col-span-4 glass-panel rounded-3xl p-5 flex flex-col justify-between border-t border-white/10 space-y-3"
        >
          <h4 className="text-[10px] font-mono uppercase tracking-[0.2em] text-prism-accent flex items-center gap-1.5">
            <Sparkles size={12} /> Why This Score?
          </h4>

          <div className="space-y-4">
            <div className="space-y-2.5">
              <h5 className="text-[9px] uppercase font-mono text-white/40 tracking-wider">Core Credibility Dimensions</h5>
              {[
                ...
                return (
                  <div key={`core-${i}`} className="space-y-1 group relative">
                    <div className="flex justify-between text-[11px] font-mono text-white/70">
                      <span className="cursor-help border-b border-white/20 border-dotted hover:text-white transition-colors" title={dim.exp}>{dim.label}</span>
                      <span className="text-white/90 font-medium">{dim.val}% ({qualityLabel})</span>
                    </div>
                    <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-1000 ${
                          dim.val >= 75 ? 'bg-cyan-400' : dim.val >= 55 ? 'bg-amber-400' : 'bg-rose-400'
                        }`}
                        style={{ width: `${dim.val}%` }}
                      />
                    </div>
            ...
            <div className="space-y-2.5 pt-2 border-t border-white/10">
              <h5 className="text-[9px] uppercase font-mono text-white/40 tracking-wider">Contextual Presentation Signals</h5>
              {[
                { label: "Bias Neutrality", val: 100 - (dimensionScores.biasLevel ?? 100), exp: "Measures neutral framing vs ideological framing. Does not inflate credibility." },
                { label: "Calmness", val: 100 - (dimensionScores.emotionalManipulation ?? 100), exp: "Measures absence of rhetorical/emotional triggers. Does not inflate credibility." }
              ].map((dim, i) => {
                return (
                  <div key={`context-${i}`} className="space-y-1 group relative">
                    <div className="flex justify-between text-[11px] font-mono text-white/50">
                      <span className="cursor-help border-b border-white/10 border-dotted hover:text-white/80 transition-colors" title={dim.exp}>{dim.label}</span>
                      <span className="text-white/60">{dim.val}%</span>
                    </div>
                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-1000 bg-white/30"
                        style={{ width: `${dim.val}%` }}
                      />
                    </div>
                  </div>
        ...
        <div className="glass-panel rounded-3xl p-5 border-t border-emerald-500/20 space-y-3">
          ...
          <ul className="space-y-2">
            {trustDrivers.map((item, i) => {
              ...
              return (
                <li key={i} className="text-xs text-white/85 flex items-start gap-2.5 bg-emerald-950/20 border border-emerald-500/15 p-3 rounded-xl">
        ...
        <div className="glass-panel rounded-3xl p-5 border-t border-amber-500/20 space-y-3">
          ...
          <ul className="space-y-2">
            {cautiousDrivers.map((item, i) => {
              const text = item.replace(/^[✓⚠]\s*/, '');
              return (
                <li key={i} className="text-xs text-white/85 flex items-start gap-2.5 bg-amber-950/20 border border-amber-500/15 p-3 rounded-xl">
          ...
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div className="flex items-center gap-3">
              <h3 className="text-xl font-light tracking-wide text-prism-text-primary flex items-center gap-2.5">
                <Search size={22} className="text-prism-accent" /> Claims Investigated
              </h3>
              <span className="text-xs font-mono font-medium px-3 py-1 rounded-full bg-prism-surface-active text-prism-accent border border-prism-accent/20">
                {claimInvestigations.length} CLAIMS CHECKED
              </span>
            </div>
            <span className="text-xs font-mono text-white/40 uppercase tracking-wider">
              Click "Show Evidence" on any claim to explore details
            </span>
          </div>
      ...
      <motion.section
        ...
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        className="glass-panel rounded-3xl p-6 border-t border-white/10 space-y-5"
      >
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs font-mono text-amber-300 bg-amber-950/40 border border-amber-500/20 px-3 py-1 rounded-full">
              Bias Level: {biasLevel}%
            </span>
            <span className="text-xs font-mono text-rose-300 bg-rose-950/40 border border-rose-500/20 px-3 py-1 rounded-full">
        ...
        {(biasIndicators.length === 0 && framingIndicators.length === 0) ? (
          <div className="flex items-center gap-4 bg-white/[0.03] border border-white/10 p-5 rounded-xl text-white/60">
      ...
      {sourceIntel && Object.keys(sourceIntel).length > 0 && (
        <motion.section
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="glass-panel rounded-2xl p-4 border-t border-white/10 space-y-3"
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
            {[
              { label: "Publisher", val: sourceIntel.publisher || "Not established", color: "text-white" },
              { label: "Source Type", val: sourceIntel.sourceType || sourceIntel.contentType || "Not established", color: "text-white" },
              { label: "Author", val: sourceIntel.author || "Not established", color: "text-white" },
              { label: "Date", val: sourceIntel.publicationDate || "Not established", color: "text-white" },
              ...
              return (
                <div key={i} className="bg-white/[0.03] border border-white/10 p-3 rounded-lg flex flex-col justify-center min-h-[60px]">
                  <span className="text-[10px] font-mono uppercase tracking-widest text-white/50 block mb-1">{item.label}</span>
                  <span className={`text-[13px] font-medium truncate block ${isMissing ? 'text-white/30 italic font-normal' : item.color}`} title={displayVal}>
                    {displayVal}
                  </span>
                </div>
              );
          ...
          {sourceIntel.evidenceProvenance && sourceIntel.evidenceProvenance !== "Not established from available content." && (
            <div className="bg-white/[0.03] border border-white/10 p-3 rounded-lg text-center mt-2">
              <span className="text-[10px] font-mono uppercase tracking-widest text-white/50 block mb-1">Evidence Provenance</span>
              <span className="text-[13px] font-medium text-white/80 block">{sourceIntel.evidenceProvenance}</span>
            </div>
          )}
          ...
          {(sourceIntel.primarySourcesReferenced?.length > 0 || sourceIntel.namedExperts?.length > 0 || sourceIntel.institutionsMentioned?.length > 0) && (
            <details className="group mt-3 bg-white/[0.02] border border-white/10 rounded-lg">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 p-4 border-t border-white/10 text-[12px] text-white/80 text-left">
      ...
      >
        ...
        <div className="space-y-2">
          {(Array.isArray(analysis.recommendations) && analysis.recommendations.length > 0
            ...
            return (
              <div key={i} className="flex items-start gap-3 bg-cyan-950/20 border border-cyan-500/20 p-4 rounded-xl text-sm text-white/90">
        ...
        {debugOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              className="bg-neutral-950 border border-white/20 rounded-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl"
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-white/5">
                <div className="flex items-center gap-3">
                  <Terminal size={18} className="text-cyan-400" />
                  <h3 className="font-mono text-sm uppercase tracking-wider text-white">Development: Raw AI Output</h3>
                </div>
                <button onClick={() => setDebugOpen(false)} className="p-1 rounded-lg text-white/60 hover:text-white hover:bg-white/10 transition-colors">
                  <X size={20} />
                </button>
              </div>
