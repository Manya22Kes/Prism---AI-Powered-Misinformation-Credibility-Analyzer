...
export const ClaimCard = (props) => {
  ...
  return (
    <Card glass hover className="mb-4 overflow-visible bg-white/[0.03] border-white/10 backdrop-blur-md">
      <div className="p-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
              <div className="flex items-center gap-2">
                ...
                {props.category && (
                  <span className="text-[10px] uppercase tracking-wider font-mono text-white/50 bg-white/5 px-2 py-0.5 rounded border border-white/10">
                    {props.category}
                  </span>
                )}
                {props.scientificConsensus && props.scientificConsensus !== 'N/A' && (
                  <span className="text-[10px] uppercase tracking-wider font-mono text-cyan-300/80 bg-cyan-950/40 px-2 py-0.5 rounded border border-cyan-500/20">
                    Consensus: {props.scientificConsensus}
                  </span>
                )}
              </div>
              <span className="text-xs text-cyan-300 font-mono bg-cyan-950/40 border border-cyan-500/20 px-2.5 py-0.5 rounded">
            ...
            {(props.reasoning || props.explanation) && (
              <p className="text-xs text-white/70 leading-relaxed bg-white/[0.02] p-3 rounded-lg border border-white/5 mb-3">
                <strong className="text-white/90 font-mono text-[11px] uppercase tracking-wider block mb-1">Analytical Reasoning:</strong>
                {props.reasoning || props.explanation}
              </p>
            )}
            ...
            <button
              ...
              className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-prism-accent hover:text-white transition-colors mt-2"
            >
              ...
              {expanded && (
                ...
                >
                  ...
                  {supporting.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {supporting.map((ev, i) => (
                          <div key={i} className="text-xs text-white/80 bg-emerald-950/20 border border-emerald-500/20 p-2.5 rounded-lg">
                            "{typeof ev === 'string' ? ev : ev.text}"
                            {ev.source && <span className="block text-[10px] text-emerald-400/60 mt-1 font-mono">Source: {ev.source}</span>}
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {contradicting.length > 0 && (
                    <div>
                      <h5 className="text-[11px] font-mono uppercase tracking-wider text-rose-400 mb-2 flex items-center gap-1.5">
                        <ShieldAlert size={14} /> Contradictory Signals & Context ({contradicting.length})
                      </h5>
                      <div className="space-y-1.5">
                        {contradicting.map((ev, i) => (
                          <div key={i} className="text-xs text-white/80 bg-rose-950/20 border border-rose-500/20 p-2.5 rounded-lg">
                            "{typeof ev === 'string' ? ev : ev.text}"
                            {ev.source && <span className="block text-[10px] text-rose-400/60 mt-1 font-mono">Source: {ev.source}</span>}
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {logicalFlaws.length > 0 && (
                    <div>
                      <h5 className="text-[11px] font-mono uppercase tracking-wider text-amber-400 mb-2">
                        Detected Logical Flaws & Weaknesses
                      </h5>
                      <div className="flex flex-wrap gap-1.5">
                        {logicalFlaws.map((flaw, i) => (
                          <span key={i} className="text-[11px] font-mono bg-amber-950/30 text-amber-300 border border-amber-500/20 px-2 py-0.5 rounded">
                            {flaw}
                          </span>
                        ))}
                      </div>
